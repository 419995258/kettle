...
public class HibernateGenericDao extends HibernateDaoSupport {
	...
	public <T> T get(Class<T> entityClass, Serializable id) {
		return (T) getHibernateTemplate().get(entityClass, id);//修改为get get不采用lazy机制  
	}
	...
	public Page pageQuery(String hql,Long totalCount, int pageNo, int pageSize,
			Object... values) {
		Assert.hasText(hql);
		Assert.isTrue(pageNo >= 1, "pageNo should start from 1");
	 ...
	 *            页号,从1开始.
	 */
	public Page pagedCacheQuery(String hql, int pageNo, int pageSize,
			Object... values) {
		Assert.hasText(hql);
		Assert.isTrue(pageNo >= 1, "pageNo should start from 1");
		// Count查询
		String countQueryString = " select count (*) "
				+ removeSelect(removeOrders(hql));
		List countlist = createQuery(countQueryString, values).setCacheable(true).list();
		long totalCount = (Long) countlist.get(0);
		...
		List list = query.setFirstResult(startIndex).setMaxResults(pageSize).setCacheable(true)
				.list();
