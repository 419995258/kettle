...
public abstract class StrutsEntityAction<T, M extends EntityDao<T>> extends
	...
	private static final long serialVersionUID = -4413342438898806976L;
	 ...
	 */
	protected void initEntity( HttpServletRequest request,
			T object) {
		bindEntity(request.getAttribute(this.getEntityName()), object);
		onInitEntity(request, object);
		if(getEntityManager() instanceof HibernateEntityDao){
			HibernateEntityDao hbEntityManager = (HibernateEntityDao)getEntityManager();
			hbEntityManager.evit(object);
		}
	}
	...
	protected Serializable[] getEntityIds(HttpServletRequest request) {
		...
		if (idString != null) {
			...
			for (int i=0;i<idString.length;i++){
				idSerializable[i]=(Serializable) ConvertUtils.convert(idString[i], idClass);
			}
