...
public class MD5BaseShortUrlService extends AbstractMapStoreShortUrlService {
	...
	protected String shorten(String longUrl,RedisManager redisManager) {
		String shortUrl = shorten(longUrl, 5,redisManager);
		if (StringUtils.isEmpty(shortUrl)) {
			shortUrl = shorten(longUrl, 6,redisManager);
		}
		return shortUrl;
	}

	public String shorten(String longUrl, int urlLength,RedisManager redisManager) {
		...
		for (int i = 0; i < 4; i++) {
			...
			if (lookupLong(shortUrl,redisManager) != null) {
				continue;
			} else {
				return shortUrl;
			}
